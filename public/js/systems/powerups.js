// ~~~~~~~~~~~~~~~~~~~~
// BUFF-URI!! acest script se ocupă de funcționabilitatea (nici eu nu știu dacă cuvântul ăsta există) buff-urilor.
// ~~~~~~~~~~~~~~~~~~~~
...
// buff-uri create de un admin

const _registeredCustom = new Set();

export function registerCustomPowerups(defs = []) {
  defs.forEach(def => {
    if (!def || !def.id || _registeredCustom.has(def.id)) return;
    const base = POWERUP_MAP[def.base];
    if (!base) return;                       

    const item = {
      ...base,
      id:       String(def.id),
      name:     String(def.name  || base.name),
      icon:     String(def.icon  || base.icon),
      price:    Math.max(1, Number(def.price) || base.price),
      category: CATS_VALID.includes(def.category) ? def.category : base.category,
      __custom: true,
      __base:   base.id,
    };

    POWERUPS.push(item);
    POWERUP_MAP[item.id] = item;
    if (CONSUMABLE_ITEMS.includes(base.id))   CONSUMABLE_ITEMS.push(item.id);
    if (PERSISTENT_ITEMS.includes(base.id))   PERSISTENT_ITEMS.push(item.id);
    if (ACTIVE_CONSUMABLES.includes(base.id)) ACTIVE_CONSUMABLES.push(item.id);
    if (PASSIVE_CONSUMABLES.includes(base.id)) PASSIVE_CONSUMABLES.push(item.id);
    _registeredCustom.add(item.id);
  });
}

const CATS_VALID = ['potions', 'combat', 'defense', 'knowledge', 'fortune'];