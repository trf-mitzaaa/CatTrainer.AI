// ~~~~~~~~~~~~~~~~~~~~
// SETĂRI!!! nu prea trebuie să explic aici, acesta este un script care permite schimbare de setări ale contului
// ~~~~~~~~~~~~~~~~~~~~
...
import { getClanById, leaveClan } from '../systems/clans.js';
...
async function doResetProgress() {
    ...
    try {
        await updateDoc(doc(db, 'users', user.uid), {
            ...
            chests: [], equipment: [], equipped: [],
            clanCoins: 0,
            missions:            { date: '', progress: {}, claimed: [] },
            clanMissionsClaimed: { date: '', ids: [] },
...
// ── Delete account ────────────────────────────────────────────────────────────
/** Scoate contul din clan înainte de ștergere (altfel rămâne membru-fantomă). */
async function leaveClanBeforeDelete(uid) {
    try {
        const snap = await getDoc(doc(db, 'users', uid));
        const clanId = snap.exists() ? snap.data().clanId : null;
        if (!clanId) return;
        const clan = await getClanById(clanId);
        if (clan && clan.members?.includes(uid)) await leaveClan(uid, clan);
    } catch (err) {
        // best-effort: dacă nu reușim, tot ștergem contul (mai bine un ghost rar decât un cont blocat)
        console.warn('[deleteAccount] curățare clan eșuată:', err);
    }
}
    ...
    if (isEmailUser()) {
        showPasswordConfirmModal(async (password) => {
            try {
                ...
                await leaveClanBeforeDelete(user.uid);
